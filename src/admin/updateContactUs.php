<?php
	...
	if (isset($_POST['submit']))
	{
		...
		if(empty($_FILES['image']['name']))
		{
			...
			{   
				header('location: updateContactUs.php?editID=1');
			}
			else
			{
		...
		else
		{
			...
			$sql = "UPDATE contactus
			SET contactLink = '$link', contactIMG = '$path'
			WHERE contactID = '$contactID'";
			mysqli_query($conn, $sql);
            ...
            <div class="container-fluid pt-4 px-4">
				<div class="bg-secondary text-center rounded p-4">
						<div class="d-flex align-items-center justify-content-between mb-4">
								<h6 class="mb-4">CONTACT US UPDATE</h6>
						</div>
										...
										if ($result -> num_rows > 0)
										{
											while ($row = $result -> fetch_assoc())
											{
												echo "<tr style = 'text-align: left;'>
														<td>".$row['contactID']."</td>
														<td>".$row['contactLink']."</td>
														<td><center><img src = ".$row['contactIMG']." style = 'width: 20%'></center></td>
														<td>
															<button><a href=\"updateContactUs.php?editID=$row[contactID]\">Update</a></button>
														</td>
													 </tr>";
											}
										}
					...
					<div class="row g-4">
						<div class="col-sm-12 col-xl-6">
							<div class="bg-secondary rounded h-100 p-4">
								<form action = "updateContactUs.php" method = "POST"  enctype="multipart/form-data">
									<?php
										global $conn;
										$editID = $_GET['editID'];
										$sql = "SELECT * FROM contactus WHERE contactID = $editID";
										...
										if ($result -> num_rows > 0)
										{
											while ($row = $result -> fetch_assoc())
											{
												$contactID = $_GET['editID'];
												$link = $row['contactLink'];
											}
										}		
											?>
									 <div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Contact ID</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "contactID" value = "<?php echo $contactID ?>" readonly>
										</div>
									</div>
									<div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Contact Lnk</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "link" value = "<?php echo $link ?>">
										</div>
									</div>
									<div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Contact Logo</label>
										<div class="col-sm-10">
