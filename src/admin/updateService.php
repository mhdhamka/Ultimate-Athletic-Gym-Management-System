<?php
	...
	if (isset($_POST['submit']))
	{
		...
		$sql = "UPDATE service
				SET serviceName = '$name', servicePrice = '$price', serviceSession = '$session', serviceDetail = '$detail'
				WHERE serviceID = '$serviceID'";
		mysqli_query($conn, $sql);
		
		if (mysqli_query($conn, $sql))
		{   
			header('location: updateService.php?editID=1');
		}
		else
		{
										...
										if ($result -> num_rows > 0)
										{
											while ($row = $result -> fetch_assoc())
											{
												echo "<tr style = 'text-align: left;'>
														...
														<td>".$row['serviceSession']."</td>
														<td>".$row['serviceDetail']."</td>
														<td>
															<button><a href=\"updateService.php?editID=$row[serviceID]\">Update</a></button>
														</td>
													 </tr>";
											}
										}
							...
							<div class="bg-secondary rounded h-100 p-4">
								<form action = "updateService.php" method = "POST"  enctype="multipart/form-data">
									<?php
										global $conn;
										$editID = $_GET['editID'];
										$sql = "SELECT * FROM service WHERE serviceID = '$editID'";
										...
										if ($result -> num_rows > 0)
										{
											while ($row = $result -> fetch_assoc())
											{
												$serviceID = $_GET['editID'];
												$name = $row['serviceName'];
												$price = $row['servicePrice'];
												$session = $row['serviceSession'];
												$detail = $row['serviceDetail'];
											}
										}		
											?>
									 <div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Service ID</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "serviceID" value = "<?php echo $serviceID ?>" readonly>
										</div>
									</div>
									<div class="row mb-3">
									...
									<div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Price</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "price" value = "<?php echo $price ?>">
										</div>
									</div>
									<div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Session</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "session" value = "<?php echo $session ?>">
										</div>
									</div>
									<div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Details</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "detail" value = "<?php echo $detail ?>">
										</div>
									</div>
