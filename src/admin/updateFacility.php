<?php
	...
	if (isset($_POST['submit']))
	{
		...
		if(empty($_FILES['image']['name']))
		{
			...
			{   
				header('location: updateFacility.php?editID=1');
			}
			else
			{
		...
		else
		{
			...
			$sql = "UPDATE facility
			SET facilityName = '$name', facilityIMG = '$path'
			WHERE facilityID = '$facilityID'"; 
			mysqli_query($conn, $sql);
            ...
            <div class="container-fluid pt-4 px-4">
				<div class="bg-secondary text-center rounded p-4">
						<div class="d-flex align-items-center justify-content-between mb-4">
								<h6 class="mb-4">FACILITY UPDATE</h6>
						</div>
										...
										if ($result -> num_rows > 0)
										{
											while ($row = $result -> fetch_assoc())
											{
												echo "<tr style = 'text-align: left;'>
														...
														<td><center><img src = ".$row['facilityIMG']." style = 'width: 30%'></center></td>
														<td>
															<button><a href=\"updateFacility.php?editID=$row[facilityID]\">Update</a></button>
														</td>
													 </tr>";
											}
										}
					...
					<div class="row g-4">
						<div class="col-sm-12 col-xl-6">
							<div class="bg-secondary rounded h-100 p-4">
								<form action = "updateFacility.php" method = "POST"  enctype="multipart/form-data">
									<?php
										global $conn;
										$editID = $_GET['editID'];
										$sql = "SELECT * FROM facility WHERE facilityID = $editID";
										...
										if ($result -> num_rows > 0)
										{
											while ($row = $result -> fetch_assoc())
											{
												$facilityID = $_GET['editID'];
												$name = $row['facilityName'];
											}
										}		
											?>
									 <div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Facility ID</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "facilityID" value = "<?php echo $facilityID ?>" readonly>
										</div>
									</div>
									<div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Facility Name</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" id="inputFname" name = "name" value = "<?php echo $name ?>">
										</div>
									</div>
									<div class="row mb-3">
										<label for="inputFname" class="col-sm-2 col-form-label">Facility Image</label>
										<div class="col-sm-10">
